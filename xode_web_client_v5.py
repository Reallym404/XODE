...
class XodeClient:
    ...
    def request_sync(self):
        ...
        # 获取最新高度：优先使用链中最后一个区块的索引
        # 如果链为空或 block_height 更高，使用 block_height
    ...
    def transfer(self, to_addr, amount):
        ...
        try:
            ...
            # 使用固定格式签名数据，确保服务端能验证
            tx_data = f"{self.wallet.address}{to_addr}{amount}"
    ...
    def _sync_missing_blocks(self, start, end):
        ...
        while current < end and self.running:
            ...
            # 等待区块到达
            wait_count = 0
            while self.chain_store.get_local_height() < batch_end - 1 and wait_count < 30:
                time.sleep(0.3)
                wait_count += 1
            current = batch_end
