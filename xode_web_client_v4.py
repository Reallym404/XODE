...
class XodeClient:
    ...
    def request_sync(self):
        ...
        if not has_genesis:
            ...
            while start <= target_height and self.running:
                ...
                start = end
                self.sync_progress = min(100, int((start / self.sync_total) * 100))
                time.sleep(0.5)
            wait_count = 0
            while self.chain_store.get_local_height() < target_height and wait_count < 50:
                ...
                if self.sync_total > 0:
                    self.sync_progress = min(100, int((self.chain_store.get_local_height() / self.sync_total) * 100))
            self.syncing = False
        ...
        if local_height < target_height:
            ...
            while start <= target_height and self.running:
                ...
                self.add_log(f"Requesting blocks #{start} to #{end-1}...")
                start = end
                progress = start - local_height - 1
                self.sync_progress = min(100, int((progress / self.sync_total) * 100))
                time.sleep(0.5)
            wait_count = 0
            while self.chain_store.get_local_height() < target_height and wait_count < 50:
                time.sleep(0.3)
                wait_count += 1
                if self.sync_total > 0:
                    current_synced = self.chain_store.get_local_height() - local_height
                    self.sync_progress = min(100, int((current_synced / self.sync_total) * 100))
            self.syncing = False
            self.sync_progress = 100
    ...
    def auto_reconnect(self, max_retries=5, delay=300):
        """自动重连，默认间隔5分钟（300秒），避免触发服务端180秒连接频率限制"""
