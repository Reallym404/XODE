...
class XodeClient:
    ...
    def _handle_node_message(self, node, msg):
        ...
        if msg_type == "pong":
            ...
            raw_issued = msg.get("issued", self.total_issued)
            # 副节点返回原子单位，主节点返回标准单位，统一转换为标准单位
            if raw_issued > TOTAL_SUPPLY * 10:
                self.total_issued = from_atomic(raw_issued)
            else:
                self.total_issued = raw_issued
            ...
            raw_burned = msg.get("burned_total", self.burned_total)
            # 副节点返回原子单位，主节点返回标准单位，统一转换为标准单位
            if raw_burned > TOTAL_SUPPLY * 10:
                self.burned_total = from_atomic(raw_burned)
            else:
                self.burned_total = raw_burned
            ...
            raw_total_issued = msg.get("total_issued", self.total_issued)
            # 副节点返回原子单位，主节点返回标准单位，统一转换为标准单位
            if raw_total_issued > TOTAL_SUPPLY * 10:
                self.total_issued = from_atomic(raw_total_issued)
            else:
                self.total_issued = raw_total_issued
    ...
    def _handle_new_block(self, msg):
        ...
        if supply:
            if "issued" in supply and supply["issued"] is not None:
                try:
                    raw_issued = float(supply["issued"])
                    # 副节点返回原子单位，主节点返回标准单位，统一转换为标准单位
                    if raw_issued > TOTAL_SUPPLY * 10:
                        self.total_issued = from_atomic(raw_issued)
                    else:
            ...
            if "burned_total" in supply and supply["burned_total"] is not None:
                try:
                    raw_burned = float(supply["burned_total"])
                    # 副节点返回原子单位，主节点返回标准单位，统一转换为标准单位
                    if raw_burned > TOTAL_SUPPLY * 10:
                        self.burned_total = from_atomic(raw_burned)
                    else:
    ...
    def transfer(self, to_addr, amount):
        ...
        try:
            ...
            # 检查余额（使用原子单位比较）
            current_balance_atomic = to_atomic(self.wallet.balance)
            if current_balance_atomic < total_atomic:
                return False, f"Insufficient balance, need {from_atomic(total_atomic)} XODE (fee {self.transfer_fee} XODE)"
            ...
            if success_count > 0:
                self.wallet.balance = from_atomic(current_balance_atomic - total_atomic)
                self.add_log(f"Local balance pre-deducted: {self.wallet.balance} XODE")
            self.wallet.save()
...
const amt=typeof r==='object'?(r.amount||perUser).toFixed(8):perUser.toFixed(8);
